...
export default class SamplingModel extends PDLModel<SamplingField> {
  ...
  public constructor( providedOptions: SamplingModelOptions ) {
    ...
    // TODO: https://github.com/phetsims/projectile-data-lab/issues/146 this code executes quickly but leads to delays on unbuilt/Chrome during startup
    // that are not seen on Firefox or on Chrome/built.  It is not clear why this is the case.
    // Also running unbuilt in Chrome with the profiling tools open makes the problem go away.
    AUTO_GENERATE_DATA_PROPERTY.link( autoGenerateData => {
      if ( autoGenerateData ) {

        // compute all fields with the same mystery launcher, unless they already have data.
        this.fields.forEach( field => {
          if ( field.launcherProperty.value === this.launcherProperty.value && field.numberOfCompletedSamplesProperty.value === 0 ) {
            field.clearProjectiles();
          }
